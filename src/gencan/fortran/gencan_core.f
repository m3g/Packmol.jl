...
C           1 means "some error occurs in the objective funtion evaluation".
      ...
      use callback_bridge_mod, only: evalfg_bridge
...
C     Modified by the Packmol.jl GENCAN FFI wrapper: computef/computeg
C     (Packmol-Fortran-specific) are replaced by a callback into Julia,
C     which computes f and g together (Packmol.jl's fg! always does both
C     in a single CellListMap pass). evalal needs only f; evalg_bridge
C     discards g into a module-level scratch array (callback_bridge_mod).

      call evalfg_bridge(n,x,f,flag)
...
C           1 means "some error occurs in the gradient evaluation".
      ...
      use callback_bridge_mod, only: evalgf_bridge
...
C     Modified by the Packmol.jl GENCAN FFI wrapper: see evalal above.
C     evalnal needs only g; evalgf_bridge discards f into a module-level
C     scratch array (callback_bridge_mod).

      call evalgf_bridge(n,x,g,flag)
