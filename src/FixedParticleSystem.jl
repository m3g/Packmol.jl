...
_copy(x) = copy(x)
Base.copy(x::OutputProperty) = OutputProperty((_copy(getfield(x, name) for name in propertynames(x)))...)

@kwdef mutable struct FixedParticleSystemOutput{T}
    overlaps_fixed::OutputProperty{Bool}
    molecule_badness::OutputProperty{Vector{T}} # output for molecule badness
end
function FixedParticleSystemOutput{T}() where {T}
    return FixedParticleSystemOutput(
        overlaps_fixed = OutputProperty(false, false),
        molecule_badness = OutputProperty(T[], false),
    )
end
...
@testitem "FixedParticleSystem" begin
    using Packmol
    using Packmol: FixedParticleSystemOutput, OutputProperty
